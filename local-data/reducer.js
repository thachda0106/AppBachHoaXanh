	...
	switch (action.type) {
		...
		case 'selectCartProduct':
			...
			for (let i = 0; i < stateTemp.currentUser.userListCart.length; i++) {
				if (stateTemp.currentUser.userListCart[i].productID === action.productID) {
		...
		case 'setCartProductQuantity':
			...
			for (let i = 0; i < stateTemp.currentUser.userListCart.length; i++) {
				if (stateTemp.currentUser.userListCart[i].productID == action.productID) {
					...
					} else if (action.mode === -1 && stateTemp.currentUser.userListCart[i].quantity > 1) {
						stateTemp.currentUser.userListCart[i].quantity -= 1;
					}
				}
			}
		...
		case 'applyVoucher':
			for (let i = 0; i < stateTemp.currentUser.userListVoucher.length; i++) {
				...
				if (stateTemp.currentUser.userListVoucher[i].voucherID == action.voucherID) {
			...
			for (let i = 0; i < stateTemp.users.length; i++) {
				if (stateTemp.users[i].userID == stateTemp.currentUser.userID) {
					for (let j = 0; j < stateTemp.users[i].userListCart.length; j++) {
						if (stateTemp.users[i].userListCart[j].productID == action.productID) {
		...
		case 'removeFromCartAll':
			stateTemp.currentUser.userListCart.splice(0, stateTemp.currentUser.userListCart.length);
			for (let i = 0; i < stateTemp.users.length; i++) {
				if (stateTemp.users[i].userID === stateTemp.currentUser.userID) {
					stateTemp.users[i].userListCart.splice(0, stateTemp.users[i].userListCart.length);
					break;
				}
			}
			return {
		...
		case 'getCurrentUser':
			...
			for (let i = 0; i < stateTemp.currentUser.userListVoucher.length; i++) {
				if (stateTemp.currentUser.userListVoucher[i].isUse.toLowerCase() === 'true')
		...
		case 'editCategory':
			...
			Function.showToast('success', 'Cập nhật danh mục sản phẩm mới thành công');
			return {
				...state,
				stateTemp
			};
			...
			let newAlert = {
				...
				content:
					action.order.orderStatus === 'DELIVERING'
						? 'Đơn hàng #' + action.order.id + ' đã được duyệt và đang được giao đến bạn'
						: 'Hoàn tất đơn hàng #' + action.order.id + '. Cảm ơn bạn đã sử dụng dịch vụ của chúng tôi',
		...
		case 'createOrder':
			...
			let newAlertCreateOrder = {
				...
				content: 'Đơn hàng của bạn đang được duyệt, chúng tôi sẽ thông báo cho bạn sớm nhất có thể',
				date: parseInt(new Date().getTime() / 1000),
				isRead: false
			};
			...
			if (action.voucherID) {
				for (let i = 0; i < stateTemp.currentUser.userListVoucher.length; i++) {
					if (action.voucherID == stateTemp.currentUser.userListVoucher[i].voucherID) {
						stateTemp.currentUser.userListVoucher[i].use = true;
						break;
					}
				}
				for (let i = 0; i < stateTemp.users.length; i++) {
					if (stateTemp.users[i].userID == stateTemp.currentUser.userID) {
						for (let j = 0; j < stateTemp.users[i].userListVoucher.length; j++) {
							if (stateTemp.users[i].userListVoucher[j].voucherID == action.voucherID) {
		...
		case 'addOrder':
			...
			for (let i = 0; i < newAddOrder.listProductCart.length; i++) {
				newAddOrder.listProductCart[i].img = action.order.listProductCart[i].picture;
			}
		...
		case 'addVoucher':
			...
			if (action.alert) {
				let newVoucherAlert = {
					...
					content: 'Có mã giảm giá mới, hãy nhanh tay thu thập và sử dụng để được nhiều ưu đã mới',
					date: parseInt(new Date().getTime() / 1000),
					isRead: false
				};
		...
		case 'addAlert':
			...
			if (action.alert.isRead.toLowerCase() === 'false') newAddAlert.isRead = false;
		...
		case 'setCategoriesFromAPI':
			...
			action.data.forEach((category, index) => {
				...
				if (newCategory.categoryImage == null) {
					newCategory.categoryImage = `https://bach-hoa.herokuapp.com/images/files/categories/${category.categoryID}`;
				}
		...
		case 'setProductsFromAPI':
			...
			action.data.forEach((product, index) => {
		  	...
		  	if (newProduct.productImage == null) {
					newProduct.productImage = `https://bach-hoa.herokuapp.com/images/files/products/${product.productID}`;
				}
		...
		case 'setUsersFromAPI':
			...
			action.data.forEach((user, index) => {
				...
				if (newUser.userImage == null) {
					newUser.userImage = `https://bach-hoa.herokuapp.com/images/files/users/${user.userID}`;
          newUser.avatar = `https://bach-hoa.herokuapp.com/images/files/users/${user.userID}`;
				}
        newUser.avatar = user.userImage;
		...
		case 'collectVoucher':
			...
			stateTemp.vouchers[index].quantity = stateTemp.vouchers[index].quantity - 1;
		...
		// thach
		case 'deleteNotify':
			state.thach.alerts = state.thach.alerts.filter((alert) => alert.alertID != action.alertID);
			return state;
