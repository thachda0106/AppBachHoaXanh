...
const Item = (props) => {
  return (
    <TouchableOpacity
      ...
      onPress={() => {
        props.navigation.navigate("OrderInfo", { order: props.order, mode: "admin" });
      }}
    >
      ...
      <View style={styles.content}>
        ...
        <Text style={styles.time}>
          {Function.timestampToDateTime(props.order.dateCreate)}
        </Text>
        <Text numberOfLines={2} ellipsizeMode="tail" style={styles.name}>
          {Function.getOrderName(props.order)}
        </Text>
      </View>
      <View style={styles.priceWrapper}>
        <Text style={styles.price}>
          {Function.toVND(Function.getOrderAllPrice(props.order))}
        </Text>
      </View>
    </TouchableOpacity>
  );
